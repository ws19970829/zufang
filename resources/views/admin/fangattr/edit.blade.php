...
@section('cnt')
    ...
    <article class="page-container">
        ...
        <form action="{{route('admin.fangattr.update',$fangattr)  }} " method="post" class="form form-horizontal" id="form-node-add">
            @csrf
            @method('PUT')
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-3">房源属性：</label>
                <div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
				<select name="pid" class="select" id="pid">
					@foreach($data as $item)
                        <option value="{{ $item['id'] }}" @if($fangattr->name==$item['name']) checked @endif >{{ $item['name'] }}</option>
                    @endforeach
				</select>
				</span></div>
            </div>
            ...
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="text" value="{{ old('field_name') }}" class="input-text" name="field_name" value="{{$fangattr->field_name}}">
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-3">个人信息图片：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <div class="uploader-thum-container">
                        <div id="filePicker">选择图片</div>
                        <input type="hidden" name="icon" id="pic" value="{{$fangattr->pic}}">
                        <div class="imgbox">
